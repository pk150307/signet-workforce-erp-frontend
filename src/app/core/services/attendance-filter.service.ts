...
export type AttendanceFilters = ModulePeriodFilters;

@Injectable({ providedIn: 'root' })
export class AttendanceFilterService {
  private readonly store = new ModuleFilterStore('attendance');
  ...
  /** Attendance screens require a client — returns empty string when unset. */
  clientIdOrEmpty = () => this.store.clientId() ?? '';
  ...
  patch(partial: Partial<AttendanceFilters>) {
